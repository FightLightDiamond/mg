...
export default class ProductChart extends React.Component {
    ...
    componentWillReceiveProps(props, state) {
        const { repost_products } = props
        let chartData = [];
        let tableData = [];
        if (repost_products.length > 0) {
            repost_products.forEach((element, index) => {
    ...
    render() {
        const data = repostProductsTransformer('Product Chart', this.state.chartData);
