...
export default class RevenueStatsChart extends React.Component {
    constructor(props) {
        super(props);
    ...
    componentWillReceiveProps(props, state) {
        const data = props.repost_revenue_stats.intervals;
        let chartData = [];
        let tableData = [];
    ...
    getData() {
        const {getReportsRevenueStats} = this.props
        const params = {
            '_locale': 'user',
    ...
    render() {
        const data = repostProductsTransformer('Revenue Chart', this.state.chartData);
        const tableColumn = this.state.columnData;
        return (
            <div style={{padding: "10px", paddingLeft: "10px"}}>
                {this.state.chartData.length > 0 &&
                <HighchartsReact highcharts={Highcharts} options={data} key={this.state.chartData}/>}
                    ...
                    <tr>
                        ...
                        {tableColumn.map((_, index) => (
                            <th key={index} onClick={() => this.sortData(tableColumn[index])}>{tableColumn[index].label}
                                {(this.state.sortColumn.value === tableColumn[index].value) && (this.state.sortColumn.order === 'asc') &&
                                <svg paddingLeft="10px" width="1.5em" height="1.5em" viewBox="0 0 16 16"
                                     className="bi bi-sort-down-alt" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                                ...
                                {(this.state.sortColumn.value === tableColumn[index].value) && (this.state.sortColumn.order === 'desc') &&
                                <svg paddingLeft="10px" width="1.5em" height="1.5em" viewBox="0 0 16 16"
                                     className="bi bi-sort-up-alt" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                                    <path fillRule="evenodd"
                                          d="M3 14a.5.5 0 0 0 .5-.5v-10a.5.5 0 0 0-1 0v10a.5.5 0 0 0 .5.5z"/>
                                    <path fillRule="evenodd"
                            ...
                            {tableColumn.map((v, i) => {
                                if (v.bind2) {
                                    if (v.bind2 === "category_ids") {
                                        return <td key={i}>
                                            <div>{value[v.bind][v.bind2].map((categoryID, index, cateArr) => {
                                    ...
                                    if (v.bind2 === "image") {
                                        return <td key={i}>
                                            <div dangerouslySetInnerHTML={{__html: value[v.bind][v.bind2]}}/>
                                        </td>
                                    }
                                    if (v.bind2 === "price") {
                                        return <td key={i}>
                                            <div>{value[v.bind][v.bind2]}{"đ"}</div>
                                        </td>
                                    }
